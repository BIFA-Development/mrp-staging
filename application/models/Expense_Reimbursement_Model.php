...
class Expense_Reimbursement_Model extends MY_Model
{
    ...
    public function __construct()
    {
        ...
        $this->module = config_item('module')['master_expense_reimbursement'];
    }
    ...
    private function searchIndex()
    {
        ...
        if (!empty($_POST['columns'][1]['search']['value'])){
            ...
            $this->db->where('tb_master_expense_reimbursement.status', $status);
        }else{
            $this->db->where('tb_master_expense_reimbursement.status', 'AVAILABLE');
        }
    ...
    {
        $this->db->select('tb_master_expense_reimbursement.*,tb_master_coa.group');
