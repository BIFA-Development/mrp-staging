...
class Reimbursement_Model extends MY_Model
{
        ...
        foreach ($document_id as $x => $id) {
            ...
            // --- JALUR 2: APPROVAL OLEH HR MANAGER (Tahap Awal) ---
                ...
                if (in_array($findDataPosition['position'], ["HEAD OF SCHOOL", "VP FINANCE", "CFO", "COO/CEO"])) {
                    $statusNew = (in_array($findDataPosition['position'], ["HEAD OF SCHOOL", "CFO"])) ? 'WAITING APPROVAL BY COO' : 'WAITING APPROVAL BY CFO';
                    $nextRoleEmail = ($statusNew == 'WAITING APPROVAL BY COO') ? 'COO/CEO' : 'CFO';
                } else {
                    $statusNew = ($spd['warehouse'] == 'JAKARTA') ? 'WAITING APPROVAL BY VP' : 'WAITING APPROVAL BY HOS';
                    $nextRoleEmail = ($spd['warehouse'] == 'JAKARTA') ? 'VP FINANCE' : 'HEAD OF SCHOOL';
                }
    ...
    /**
     * Helper internal untuk merapikan insert signer (Opsional agar kode approve tidak terlalu panjang)
     */
    private function insert_signer_log($id, $doc_num, $action, $notes)
    {
        $this->db->set('document_type', 'RF');
        $this->db->set('document_number', $doc_num);
        $this->db->set('document_id', $id);
...
{
    // Cari semua ER yang merujuk ke ID Reimbursement ini
    ...
    if (count($results) > 1) {
        ...
        // Loop mulai dari indeks 1 (Menyisakan indeks 0)
        for ($i = 1; $i < count($results); $i++) {
            $duplicate_er = $results[$i];
            $er_id = $duplicate_er->id;

            // Reversal Budget (MTD & YTD)
            $used_budgets = $this->connection->get_where('tb_expense_used_budgets', ['expense_purchase_requisition_id' => $er_id])->result();
        ...
        // UPDATE PENTING: Pastikan pr_number merujuk ke ER yang kita sisakan (index 0)
        ...
        if ($this->connection->trans_status() === FALSE || $this->db->trans_status() === FALSE) {
            ...
            $this->connection->trans_commit();
            $this->db->trans_commit();
            return "Sukses! Budget ID $reimbursement_id telah diperbaiki. Duplikat dihapus.";
        }
    }
    return "Tidak ditemukan data duplikat untuk ID ini.";
}
}
