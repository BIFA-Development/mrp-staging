...
class Amount_Leave extends MY_Controller
{
    protected $module;
    ...
    public function __construct()
    {
        ...
        $this->module = $this->modules['master_amount_leave'];
    ...
    public function index()
    {
        ...
        $this->data['page']['title']        = 'Amount Leave Items';
    ...
    public function edit($id)
    {
        ...
        $_SESSION['amount_leave']['id'] = $entity['id'];
        $_SESSION['amount_leave']['amount_leave'] = $entity['amount_leave'];
        $_SESSION['amount_leave']['position'] = $entity['position'];
        $_SESSION['amount_leave']['leave_id'] = $entity['leave_id'];
        ...
        if (is_granted($this->module, 'save') === FALSE){
            ...
            if ($this->input->post('id') != ''){
                ...
                if ($this->model->update($position_data, $criteria)){
                    $return['success'] = TRUE;
                    $return['message'] = 'Amount leave item ' . $this->input->post('amount_leave') .' updated.';
                } else {
                    $return['success'] = FALSE;
                    $return['message'] = 'There are error while updating data. Please try again later.';
                }
            } else {
                // if ($this->model->isDuplicateBenefitType($this->input->post('benefit_type'))){
                //     $return['success'] = FALSE;
                //     $return['message'] = 'Benefit Type! '. $this->input->post('benefit_type') .' already exists.';
                // } else {
                    ...
                    if ($this->model->insert($position_data)){
                        $return['success'] = TRUE;
                        $return['message'] = 'Amount Leave Item for ' . $this->input->post('position') .' created.';
                    } else {
                        $return['success'] = FALSE;
                        $return['message'] = 'There are error while updating data. Please try again later.';
                    }
