...
class Benefit_Type extends MY_Controller
{
    protected $module;
    ...
    public function __construct()
    {
        ...
        $this->module = $this->modules['master_benefit_type'];
    ...
    public function index()
    {
        ...
        $this->data['page']['title']        = 'Benefit Type';
        ...
        if (is_granted($this->module, 'index') === FALSE){
            ...
            $no   = $_POST['start'];
              ...
              foreach ($entities as $row){
                ...
                $col[] = print_string($row['benefit_type']);
                $col[] = print_string($row['notes']);
    ...
    public function edit($id)
    {
        ...
        $_SESSION['benefit_type']['id'] = $entity['id'];
        $_SESSION['benefit_type']['benefit_type'] = $entity['benefit_type']; 
        ...
        if (is_granted($this->module, 'save') === FALSE){
            ...
            if ($this->input->post('id') != ''){
                ...
                if ($this->model->update($position_data, $criteria)){
                    $return['success'] = TRUE;
                    $return['message'] = 'Benefit Type ' . $this->input->post('benefit_type') .' updated.';
                } else {
                ...
                if ($this->model->isDuplicateBenefitType($this->input->post('benefit_type'))){
                    ...
                    if ($this->model->insert($position_data)){
                        $return['success'] = TRUE;
                        $return['message'] = 'Benefit Type for ' . $this->input->post('benefit_code') .' created.';
                    } else {
                        $return['success'] = FALSE;
                        $return['message'] = 'There are error while updating data. Please try again later.';
                    }
